...
local imgFiles = {"blue_crystal1.png","orange_crystal2.png"}

local homeURL = "https://auroras.fusionbombsderp.com"
local saveFile = "AuroraOasisSave.id"

local token = composer.getVariable( "token" )
...
local function networkListenerGetCrystals( event )
    ...
    if ( event.isError ) then
		...
		for i=1,table.getn(decode) do
			-- print(i)
			local line = decode[i]
			table.insert(markers, {line.lat, line.long, line.type})
		end
...
function scene:show( event ) 
	...
	elseif ( phase == "did" ) then 
		...
		-- userIDText:setFillColor( 0, 0, 1 )

		-- part of the map will go below the standard viewport
		myMap = native.newMapView(0, 0,display.contentWidth , display.contentHeight -60) 
		if myMap then 
			myMap.mapType = "standard" 
			myMap.x = display.contentCenterX 
			-- navBar height is 52 per api doc // deleted!
			myMap.y = display.contentCenterY - 140
